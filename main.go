...
func (w Walking) Calories() float64 {
	...
	timeTraining := w.Duration.Hours() * MinInHours
	return (CaloriesWeightMultiplier*w.Weight + (math.Pow(meanSpeedMsec, 2)/(w.Height/CmInM))*CaloriesSpeedHeightMultiplier*w.Weight) * timeTraining

	//return (CaloriesWeightMultiplier*w.Weight + (math.Pow(w.meanSpeed(), 2)*KmHInMsec/(w.Height/CmInM))*CaloriesSpeedHeightMultiplier*w.Weight) * float64(w.Duration/time.Hour) * MinInHours
}
...
func (s Swimming) distance() float64 {
	// вставьте ваш код ниже
	l := float64(s.LengthPool*s.CountPool) / MInKm
	return l
}
